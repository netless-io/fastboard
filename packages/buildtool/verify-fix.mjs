...
const hasFix =
  /if\s*\(typeof\s+define\s*==\s*["']function["']\s*&&\s*define\.amd\)\s*\{[\s\S]*?define\s*\(function\s*\(\)\s*\{[\s\S]*?return\s+Decimal;[\s\S]*?\}\);[\s\S]*?module2\.exports\s*=\s*Decimal;[\s\S]*?\}\s*else\s*if/.test(
    code
  );
...
if (hasFix) {
  ...
  // 显示相关代码片段
  const defineAmdMatch = requireDecimalCode.match(
    /if\s*\(typeof\s+define\s*==\s*["']function["']\s*&&\s*define\.amd\)\s*\{[\s\S]*?\}\s*else\s*if/
  );
  if (defineAmdMatch) {
    console.log("\n当前代码片段:");
